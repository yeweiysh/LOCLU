...
for i=1:1
    ...
    for j=[20]
        ...
        n=size(label,1);
        numClus=4;
        clustersize1=length(find(label(:,1)==1));
        clustersize2=length(find(label(:,1)==2));
        clustersize3=length(find(label(:,1)==3));
        clustersize4=length(find(label(:,1)==4));
        
        index1=zeros(clustersize1,1);
        index2=zeros(clustersize2,1);
        index3=zeros(clustersize3,1);
        index4=zeros(clustersize4,1);
        
        r=randperm(clustersize1);
        point=floor(clustersize1/2);
        index1(r(1:point),1)=1;
        index1(r(point+1:end),1)=2;
        
        r=randperm(clustersize2);
        point=floor(clustersize2/2);
        index2(r(1:point),1)=3;
        index2(r(point+1:end),1)=4;
        
        r=randperm(clustersize3);
        point=floor(clustersize3/2);
        index3(r(1:point),1)=5;
        index3(r(point+1:end),1)=6;
        
        r=randperm(clustersize4);
        point=floor(clustersize4/2);
        ...
        community(:,1)=1:size(label,1);
        community(:,2)=newlabel;
        filename=['C:\Users\Desktop\local clustering\synthetic graph data\intro\' num2str(i) '\' num2str(j) '\community.txt'];
        fileID = fopen(filename,'w');
        [row,col]=size(community);
        for ii=1:row
            for jj=1:col
                fprintf(fileID, '%d ', community(ii,jj));
            end
            fprintf(fileID, '\n');
        end
        fclose(fileID);
