...
% Minor changed by Wei Ye (ye@dbs.ifi.lmu.de).
%------------

function	[dip,xl,xu,triple,gcm, lcm, mn, mj,ifault] = HartigansDipTest(xpdf)
...
while itarate_flag
    ...
    if itarate_flag
        ...
        if du_best<du
            du_best=du;
            triple_du_best=triple_du;
        end
        % determine the current maximum
        % CODE BREAK POINT 90
        if dl_best>du_best
            triple=triple_dl_best;
        else
            triple=triple_du_best;
        end
